...
export default function App() {
  ...
  const [recovery, setRecovery] = useState(false); // true after clicking a reset link
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, s) => {
      ...
      if (event === "PASSWORD_RECOVERY") {
        setRecovery(true);
        setAuthError(""); setAuthNotice(""); setAuthPassword("");
        setShowProfile(true);
      }
    });
  ...
  async function sendReset() {
    setAuthError(""); setAuthNotice("");
    if (!authEmail.trim()) { setAuthError(t("Indtast din email.")); return; }
    setAuthBusy(true);
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(authEmail.trim(), { redirectTo: window.location.origin });
      if (error) throw error;
      setAuthNotice(t("Vi har sendt et link til at nulstille din adgangskode."));
    } catch (err) {
  ...
  async function updatePassword() {
    setAuthError(""); setAuthNotice("");
    if (authPassword.length < 6) { setAuthError(t("Adgangskode skal være mindst 6 tegn.")); return; }
    setAuthBusy(true);
    try {
      ...
      setRecovery(false);
      setAuthPassword("");
      setAuthNotice(t("Din adgangskode er opdateret."));
    } catch (err) {
      setAuthError(friendlyAuthError(err));
    } finally {
      setAuthBusy(false);
      ...
      <div className="profile-sheet" onClick={e => e.stopPropagation()}>
        ...
        {!isCloudConfigured ? (
          ...
          <div className="profile-body">
            <div className="profile-email-label">{t("Nulstil adgangskode")}</div>
            <form className="profile-form" onSubmit={e => { e.preventDefault(); updatePassword(); }}>
              <label className="profile-field">
                <span className="profile-field-label">{t("Ny adgangskode")}</span>
                <input className="profile-input" type="password" autoComplete="new-password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} />
              </label>
          ...
          <div className="profile-body">
            {isGoogleEnabled && (
              <>
                <button className="profile-google-btn" type="button" onClick={googleSignIn}>
                  ...
                  {t("Fortsæt med Google")}
                </button>
                <div className="profile-or"><span>{t("eller")}</span></div>
              </>
            )}
            ...
            {authMode === "login" && (
              <button className="profile-forgot" type="button" onClick={sendReset} disabled={authBusy}>
                {t("Glemt adgangskode?")}
              </button>
            )}
